package module.mihoyo.armsinfo;
...
public class ArmsInfo {
    ...
    public void armsInfo(MessageGet msg) {
        ...
        final String arms = msg.getText().split("信息")[0];//从消息体中截取武器名称
        for (String a : armsList) {//构建图片并发送图片消息
            if (a.equals(arms))
                send.groups((GroupMsg) msg, factory.getMessageContentBuilder().image(Objects.requireNonNull(this.getClass().getResourceAsStream(String.format("/arms-pic/%s.png", a)))).build());
        }
    }
}
